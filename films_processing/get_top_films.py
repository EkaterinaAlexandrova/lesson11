...
def get_movie_description(rating):
    movies = [movie for movie in imdb_rank if float(movie['rating']) >= rating]
    if len(movies) == 0:
        return "No movies with that rating found."
    else:
...
description = get_movie_description(8.5)
print(description)
counter=count_words(description)
print(counter)